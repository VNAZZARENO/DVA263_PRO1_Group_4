...
def model_pipeline(df, 
    ...
    if subset_frac:
        ...
        single_instance = X_numeric[y == 2]
        for _ in range(9):
            X_numeric = pd.concat([X_numeric, single_instance], ignore_index=True)
            y = pd.concat([y, pd.Series([2] * len(single_instance))], ignore_index=True)
